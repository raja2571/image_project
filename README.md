...
1. Read product images.
2. Detect and recognize text present in the images.
3. Extract the relevant entity/value from the OCR output.
4. Identify the numerical value and its corresponding unit.
5. Normalize unit abbreviations into standard unit names.
6. Store the extracted information in a structured CSV dataset.
7. Process a large number of images efficiently.
8. Preserve processing progress so that the system can resume after interruption.
...
*Solution:* EasyOCR is used to perform Optical Character Recognition.
...
The project therefore divides processing into batches. Current implementation:
...
The project checks whether an existing progress CSV is available and resumes processing from unprocessed records — a checkpoint-based processing mechanism.
...
The extracted abbreviation is then converted into its complete unit name. For example:
...
**Raw Text** — the original OCR output:

```python
raw_text
```

**Cleaned Text** — special/non-alphanumeric characters are removed while retaining alphabetic characters, numbers, decimal points, and spaces:
...
The system first checks whether the image exists. If the image is missing:
...
Because the project is designed for a large number of images, processing is divided into batches. The configured batch size is:
...
The project contains a resume mechanism. The output file is:
...
Before starting, the system checks whether the progress file already exists. If it exists:
...
This is particularly useful in Google Colab because runtime sessions can disconnect. The notebook explicitly implements this checkpoint mechanism.
...
- Python — used for data processing, image processing, OCR, regular expressions, CSV manipulation, and file management.

**Development Environment**
- Google Colab — the project was implemented in Google Colab using the Python 3 kernel, configured to use GPU acceleration.

**OCR Framework**
- EasyOCR — used for text detection, text recognition, and image-to-text conversion. The notebook installs EasyOCR and initializes an English OCR reader.

**Data Processing**
- Pandas — used for reading CSV files, managing datasets, adding columns, updating rows, and saving processed data.

**Image Processing**
- OpenCV (`cv2`) — installed and imported alongside EasyOCR and Pandas.

**Regular Expressions**
- Python's `re` module — used for text cleaning, number detection, unit detection, and entity-value extraction.

**Storage**
- Google Drive — used to store input images, input CSV datasets, intermediate processing data, and the final output CSV. The notebook mounts Google Drive using `drive.mount('/content/drive')`.
...
The notebook is configured with GPU acceleration. A GPU runtime is useful because OCR over thousands of images can be computationally intensive.
...
4. Upload/store the image dataset.
5. Upload/store the input CSV.
6. Update the paths in the notebook.
7. Install dependencies.
8. Initialize EasyOCR.
9. Run the processing cells.
10. Wait for batch processing to complete.
11. Retrieve the generated CSV from Google Drive.
